...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const fitmentData = {
        chevrolet: {
            ...
            vehicles: [
                ...
                { vehicle: "Blazer", years: "2019+", lugPattern: "6x120", cb: "74.1mm", thread: "M14x1.5", offset: "+45 to +52", notes: "" }, // Corrected, Blazer is 6x120 for newer
        ...
        ford: {
            ...
            vehicles: [
                ...
                { vehicle: "Explorer", years: "2020+", lugPattern: "5x114.3", cb: "67.1mm", thread: "M14x1.5", offset: "+40 to +50", notes: "Older can be 5x114.3 w/ 70.5mm CB" }, // Corrected CB for newer Explorer
        ...
        toyota_lexus: {
            ...
            description: "Tacoma, Camry, RX350, IS250, etc.",
            tag: "Japanese Import",
            vehicles: [
    ...
    const setActivePage = (pageId) => {
        ...
        navLinks.forEach(link => {
            // Check if the link is part of the hero button, if so, don't try to mark it as active nav link
            if (link.classList.contains('hero-cta-button')) {
                 // Potentially highlight the target nav link if needed, but hero button itself isn't a nav item
                const targetNav = document.querySelector(`.app-nav a[data-page="${pageId}"], .mobile-bottom-nav a[data-page="${pageId}"]`);
                if(targetNav) targetNav.classList.add('active-nav-link');
                return; 
            }
    ...
    const populateManufacturerCards = () => {
        ...
        manufacturerCardsList.innerHTML = ''; // Clear existing
    ...
    const filterManufacturerCards = () => {
        ...
        cards.forEach(card => {
            if (card.dataset.searchTerms.includes(searchTerm)) {
                card.classList.remove('hidden-card'); // This class should be in CSS: .hidden-card { display: none !important; }
            } else {
    ...
    const showFitmentDetail = (mfgKey) => {
        ...
        fitmentDetailTableContainer.innerHTML = ''; // Clear previous
        const table = createFitmentTable(mfg.vehicles);
        fitmentDetailTableContainer.appendChild(table);

        // Hide card list and search, show detail view
        if(document.getElementById('fitment-page').querySelector('.fitment-search-container')) {
            document.getElementById('fitment-page').querySelector('.fitment-search-container').style.display = 'none';
        }
    ...
    const hideFitmentDetail = () => {
        ...
        // Show card list and search
        if(document.getElementById('fitment-page').querySelector('.fitment-search-container')) {
            document.getElementById('fitment-page').querySelector('.fitment-search-container').style.display = 'block';
        }
    ...
    navLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            if (pageId === 'fitment-page') { 
                hideFitmentDetail(); // When navigating to fitment page, always show list view first
            }
        });
    });
    ...
    populateManufacturerCards(); // Create the manufacturer cards
    setActivePage('home-page'); // Default to home page
});
